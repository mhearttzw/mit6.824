...
import "github.com/mhearttzw/mit/src/linearizability"
...
func run_client(t *testing.T, cfg *config, me int, ca chan bool, fn func(me int, ck *Clerk, t *testing.T)) {
	ok := false
	defer func() { ca <- ok }()
	// create a clerk to call servers's method
...
/**
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			// key equals to key, last equals to value
			Put(cfg, myck, key, last)
			DPrintf("GenericTest call Put method over, round %v", i)
...
// ncli means n clients;
func spawn_clients_and_wait(t *testing.T, cfg *config, ncli int, fn func(me int, ck *Clerk, t *testing.T)) {
	ca := make([]chan bool, ncli)
	for cli := 0; cli < ncli; cli++ {
		ca[cli] = make(chan bool)
		// cli equal to fn's me parameter, one of n clients;
		// ca[cli] means client's channel used to send msg when call finished.
