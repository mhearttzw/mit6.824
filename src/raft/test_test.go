...
func TestBasicAgree2B(t *testing.T) {
	...
	// submit sequence cmd in raft servers
	iters := 3
	for index := 1; index < iters+1; index++ {
		...
		// try all raft servers to submit the cmd and check log replication.
		xindex := cfg.one(index*100, servers, false)
		// make sure that only this test submits the command to raft server
		// there is a situation that leader fails just rf.start() which could result in xindex != index
		if xindex != index {
...
func TestFailAgree2B(t *testing.T) {
	servers := 3
	// create a full of raft instances
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
...
// output result:
// these are the time that the test took in seconds,
// the number of Raft peers (usually 3 or 5),
// the number of RPCs sent during the test,
// and the number of log entries that Raft reports were committed.
...
loop:
	...
	fmt.Printf("RPCS (%v) FOR 1 second of idleness\n", total3-total2)
...
func TestPersist12C(t *testing.T) {
	servers := 3
	// unreliable: simulate the timout network
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
