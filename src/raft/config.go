...
// combine crand.Ind() and math.Seed() to create real rand number
func makeSeed() int64 {
...
func make_config(t *testing.T, n int, unreliable bool) *config {
	...
	cfg := &config{}
	cfg.t = t
	cfg.net = labrpc.MakeNetwork()		// MakeNetwork kick off goroutine that handles all raft server rpc and dispatch rpc request background.
	cfg.n = n
	...
	// connect everyone
	// outgoing clientnames and incoming clientnames
	for i := 0; i < cfg.n; i++ {
		cfg.connect(i)
	}
...
func (cfg *config) start1(i int) {
	...
	DPrintf("Make raft instance: %v", i)
...
func (cfg *config) one(cmd int, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		if index != -1 {
			...
			// wait for raft handling the log replication
			t1 := time.Now()
			var nd int
			var cmd1 interface{}
