...
func doMap(
	...
	mapTask int, // which map task this is (also the index of files)
	...
	// 2. store reduce immediate file handles
	var rFiles []*os.File
	for r := 0; r < nReduce; r++ {
		rFileName := reduceName(jobName, mapTask, r)
	...
	// 3. write data into reduce file
	for _, kv := range kvs {
		// mod key to choose which file to store
		r := ihash(kv.Key) % nReduce
		enc := json.NewEncoder(rFiles[r])
		err := enc.Encode(&kv)
		if err != nil {
