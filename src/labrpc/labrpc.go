...
import "github.com/mhearttzw/mit/src/labgob"
import "bytes"
import "reflect"
import "sync"
import "log"
...
func MakeNetwork() *Network {
	...
	rn.enabled = map[interface{}]bool{}		// whether client end is enable
...
func (rn *Network) MakeEnd(endname interface{}) *ClientEnd {
	...
	e := &ClientEnd{}
	e.endname = endname
	// Client end's req channel and done channel is same to the Network's.
	e.ch = rn.endCh
	e.done = rn.done
	rn.ends[endname] = e
...
func (rs *Server) dispatch(req reqMsg) replyMsg {
	...
	// req.svcMeth, e.g. "Raft.AppendEntries"
...
func (svc *Service) dispatch(methname string, req reqMsg) replyMsg {
	if method, ok := svc.methods[methname]; ok {
		...
		// decode the argument into local variable.
